...
document.addEventListener("click", function (e) {
    ...
    if (targetElement.closest(".slide-team")) {
        const slideTeam = targetElement.closest(".slide-team");
    ...
    if (targetElement.closest(".owner-team__image")) {
        const ownerTeam = targetElement.closest(".owner-team__image");
    ...
    if (targetElement.closest(".team__image")) {
        const teamItem = targetElement.closest(".team__image");
    ...
    if (targetElement.closest(".menu-header__link")) {
        localStorage.removeItem("current-card");
    };


    // "Клик" по кнопке "читать полностью" в блоке "Статьи" на главной странице:
    if (targetElement.closest(".item-article__button") && !document.querySelector("._page-articles")) {
        const currentCard = targetElement.getAttribute("data-scroll-elem");
        currentCard ? localStorage.setItem("current-card", currentCard) : null;
    }
});
...
function gotoBlock(e) {
    ...
    if (targetBlockElement) {
        ...
        if (document.documentElement.classList.contains("_menu-open")
            || document.querySelector(".menu-header").classList.contains("open")) {
            closeMenu(menu, iconMenu)
            toggleLockBody(true, fixedElements, delay, bodyScrollBar);
        }
...
window.onload = function () {
    ...
    if (document.querySelector("._page-about")) {
        ...
        document.body.classList.contains("_touch") ? timeout = 500 : timeout = 100;
        if (elemId) {
...
if (document.querySelector("header[data-scroll]")) {
    ...
    var isShow = header.hasAttribute("data-scroll-show") ? true : false;
    var scrollPoint = header.dataset.scroll ? header.dataset.scroll : 1;
    var scrollTop = window.scrollY;
    var scrollValue, scrollDirection, showPoint, showTimer, timer;
...
//     });
// }
if (document.querySelector(".page-articles")) [
    $('.item-article__statistic button').on('click', function (e) {
        var _this = this;
        var span = $(this).parent().find('span');
        var currentLikes = parseInt(span.html());
        var id = span.data('vote-id');
        var action = $(this).parent().find('span').attr('data-action');
        $.ajax({
            ...
            success: function (data) {
                if (data.status === 'success' && data.action === 'add') {
                    $(_this).parent().find('span').attr("data-action", "delete");
                    $(_this).parent().find('span').html(currentLikes + 1);
                    $(_this).addClass('active');
                }
                if (data.status === 'success' && data.action === 'delete') {
                    $(_this).parent().find('span').attr("data-action", "add");
                    $(_this).parent().find('span').html(currentLikes - 1);
...
if (document.querySelector("[data-form]")) {
    ...
    async function formSend(e) {
        ...
        const formImage = form.querySelector("[type='file']");
        const filePreview = form.querySelector(".form-file__preview");
        const formCommunication = form.getAttribute("data-form");
        if (formImage?.files[0]) {
            formData.append("image", formImage.files[0]);
        }
        if (formCommunication) {
            formData.append("communication", formCommunication);
        }
        if (error === 0) {
            socialInput.value = "";
            ...
            form.closest(".contacts__column").classList.add("form-send");
            form.reset();
            filePreview ? filePreview.innerHTML = "" : null;
            if (response.ok) {
                form.closest(".contacts__column").classList.remove("form-send");
...
if (document.querySelector("[data-share]")) {
    ...
    $('button.item-article__share').on('click', function () {
        if ($(this).attr('data-add-statistic') === '') {

            $(this).removeAttr('data-add-statistic');
            var id = $(this).closest('.page-articles__item').data('id');
            var currentShareCount = parseInt($(this).closest('.page-articles__item').find('.item-article__statistic').find('.share__counter').html());

            var _target = this;

            $.ajax({
                url: '/ajax/ajax-share.php',
                method: 'post',
                dataType: 'json',
                data: { action: 'add', id: id },
                success: function (data) {
                    if (data.status === 'success') {
                        $(_target).closest('.page-articles__item').find('.item-article__statistic').find('.share__counter').html(currentShareCount + 1)
                    }
                }
            });
        }
